...
struct mycompare
{	
	//greater function for priority queue, bigger height first, if equal height, bigger ending time first 
...
vector<pair<int, int> > getSkyline(vector<vector<int> >& buildings) 
{
    ...
    priority_queue< pair<int, int>, vector<pair<int, int> >, mycompare> liveBlg; 
	
	while(cur < len || liveBlg.empty() == false)
    { 
    	...
    	{ 
    	    ...
    	    while(liveBlg.empty() == false && (liveBlg.top().second <= cur_X) )
			{
				liveBlg.pop();
			}
    	}
...
//divide and conqueor approach will be added
